...
usage() {
    ...
    echo "  -r, --role ROLE          Graph permission role to remove (e.g., Mail.Send, User.Read.All)"
...
if [[ -z "$existingAssignment" ]]; then
    echo "Role '$ROLE' is not assigned to user '$USER_EMAIL'"
    exit 0
else
    echo "Removing role '$ROLE' from user '$USER_EMAIL'..."
    az rest --method DELETE --uri "https://graph.microsoft.com/v1.0/users/$userId/appRoleAssignments/$existingAssignment"
